...
export default {
  ...
  methods: {
    ...
    // 保存员工基本信息
    async saveUser() {
      const fileList = this.$refs.staffPhoto.fileList // 读取上传组件的数据 数组 内部url对象
      if (fileList.some(item => !item.upload)) {
        // 此时表明还未上传成功
        this.$message.warning('您当前图片还没有上传完成')
        return
      }
    ...
    // 保存员工详情的基本信息
    async savePersonal() {
      const fileList = this.$refs.mystaffPhoto.fileList // 读取上传组件的数据 数组 内部url对象
      if (fileList.some(item => !item.upload)) {
        // 此时表明还未上传成功
        this.$message.warning('您当前图片还没有上传完成')
        return
